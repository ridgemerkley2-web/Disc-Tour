"""Strict, read-only Session 3 animation/montage/ABP validation.

Run with UnrealEditor-Cmd's PythonScript commandlet. The script hashes every
Session 2 foundation package plus all Session 3 animation outputs before and
after reflected inspection, proving that validation did not persist a change.
Only the JSON evidence report under Saved is written.
"""
...
REPORT_PATH = PROJECT_ROOT / "Saved" / "CharacterFramework" / "Session3AssetValidation.json"
...
def _hash_packages():
    ...
    for relative in PROTECTED_PACKAGES:
        ...
        if not path.is_file():
            raise RuntimeError(f"Required Session 3 package is missing: {path}")
        raw = path.read_bytes()
        result[relative] = {
...
def main():
    ...
    report = json.loads(
        unreal.DiscGolfSession3AssetUtility.validate_session3_assets()
    )
    if not str(report.get("status", "")).startswith("PASS"):
        raise RuntimeError(report.get("error", "Session 3 strict asset validation failed"))
    ...
    if changed:
        raise RuntimeError(
            f"Read-only Session 3 validation changed packages on disk: {changed}"
        )
    ...
    unreal.log(
        "DG_SESSION3_ASSET_VALIDATION: PASS "
        "duration=2.8s fps=60 release=1 finish=1 "
        "ABP=RefPose_DefaultSlot_Root disk_mutation=NONE"
    )
