    ...
    ProveCreatorAndPersistenceContracts(FString& OutError)
{
    ...
    {
        OutError = TEXT("applied schema-9 character did not reload or reopen");
        return false;
    }
    ...
    ProveSchema8Migration(FString& OutError)
{
    ...
    {
        OutError = TEXT("schema 8 did not migrate legacy body/outfit into deterministic schema-9 defaults");
        return false;
    }
    const UDiscGolfSaveGame* Reloaded = Cast<UDiscGolfSaveGame>(
        UGameplayStatics::LoadGameFromSlot(ValidationSaveSlot, 0));
    ...
    FString& OutError)
{
    ...
    Command.MoldId = TEXT("Session7VisualNoGameplayDisc");
