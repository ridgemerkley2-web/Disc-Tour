...
bool BindSession5SelectedThrowProvenance(
...
bool ADiscGolfSession5MocapVisualCaptureRunner::BeginProfileThrows()
{
    ...
    {
        FThrowCommand BoundFixtureCommand = FixtureCommand;
        if (!ProfilePawns[Index]
            || !ProfileAdapters[Index]
            || !BindSession5SelectedThrowProvenance(
                ProfilePawns[Index], BoundFixtureCommand))
        {
            return false;
        }
        FixtureCommands.Add(BoundFixtureCommand);
        if (!ProfilePawns[Index]->TryStartAnimatedRHBHThrow(BoundFixtureCommand)
            || !ProfileAdapters[Index]->IsThrowActive()
            || !Session5CommandsMatchExactly(
                ProfileAdapters[Index]->GetLastAuthoritativeCommand(),
                BoundFixtureCommand))
        {
            return false;
        }
...
{
    const bool bExactFixtureCommand = FixtureCommands.ContainsByPredicate(
        [&AuthoritativeCommand](const FThrowCommand& Expected)
        {
            return Session5CommandsMatchExactly(AuthoritativeCommand, Expected);
        });
    if (!bExactFixtureCommand
