...
const TCHAR* ProductionMotionCaptureFilenames[] = {
    TEXT("01_Aim_Baseline_SliderMax.png"),
    TEXT("02_ReachBack_Baseline_SliderMax.png"),
    TEXT("03_Plant_Baseline_SliderMax.png"),
    TEXT("04_Release_Baseline_SliderMax.png"),
    TEXT("05_FollowThrough_Baseline_SliderMax.png"),
    TEXT("06_Recovery_Baseline_SliderMax.png")
};

const TCHAR* ProductionMotionRequiredPhases[] = {
    TEXT("Aim"),
    TEXT("ReachBack"),
    TEXT("Plant"),
    TEXT("Release"),
...
bool BindSession4SelectedThrowProvenance(
...
bool Session4CommandsMatchExactly(
    const FThrowCommand& A,
    const FThrowCommand& B)
{
...
void ADiscGolfSession4VisualCaptureRunner::Start()
{
    ...
    bStarted = true;
    bProductionMotionVisualCapture = FParse::Param(
        FCommandLine::Get(),
        TEXT("Session19ProductionMotionVisualCapture"));
    if (bProductionMotionVisualCapture)
    {
    ...
    if (!GameMode || !PlayerController || !PossessedGolfer
        || (!bProductionMotionVisualCapture && !ThrowMontage)
    ...
    OutputDirectory = FPaths::Combine(
        ...
        bProductionMotionVisualCapture
            ? TEXT("CharacterFramework/Screenshots/Session19_ProductionMotion")
            : TEXT("CharacterFramework/Screenshots/Session4_CharacterCreator"));
    ManifestPath = FPaths::Combine(
        OutputDirectory,
        bProductionMotionVisualCapture
            ? TEXT("Session19_ProductionMotion_CaptureManifest.json")
    ...
    if (bProductionMotionVisualCapture)
    {
        ...
        {
            Fail(TEXT(
                "production motion capture requires D3D12, exact 1920x1080 ForceRes, and all no-load/no-write save guards; NullRHI is forbidden"));
            return;
        }
    }
    ...
    if (!PreflightAssets() || !CreateCaptureCamera())
    {
        if (!bFinished)
        {
    ...
    const bool bFixturesSpawned = bProductionMotionVisualCapture
        ? SpawnProductionMotionFixtures()
        : SpawnRegularFixtures();
    if (!bFixturesSpawned
        || (!bProductionMotionVisualCapture && !RunHandednessBoundaryAssertion()))
    {
        if (!bFinished)
        {
            Fail(TEXT("Session 4 visual fixture setup failed"));
        }
        return;
    }
    ...
    UE_LOG(LogDiscGolfTour, Display,
        TEXT("DG_SESSION4_VISUAL_CAPTURE: START mode=%s expected_revision=%s output=%s"),
        bProductionMotionVisualCapture ? TEXT("production_motion") : TEXT("session4"),
...
void ADiscGolfSession4VisualCaptureRunner::Tick(float DeltaSeconds)
{
    ...
    switch (Stage)
    {
        case EStage::WaitingForNeutralFront:
            if (SecondsInStage() >= PoseSettleSeconds)
            {
                ...
                if (bProductionMotionVisualCapture)
                {
                    ...
                    {
                        Fail(TEXT("production Drive montages did not start in exact Aim phase"));
                        return;
                    }
                    PrepareAimCapture();
                }
        ...
        case EStage::WaitingForReachback:
            if (bProductionMotionVisualCapture
                ? (AreFixturesAtPhase(EDGThrowPhase::ReachBack)
                    && GetMinimumCurveValue(TEXT("DG_ReachbackAlpha")) >= 0.90f)
                : AreFixturesAtMontageTime(ReachbackSeconds))
            {
        ...
        case EStage::WaitingForPlant:
            if (bProductionMotionVisualCapture
                ? (AreFixturesAtPhase(EDGThrowPhase::Plant)
                    && GetMinimumCurveValue(TEXT("DG_BraceAlpha")) >= 0.85f
                    && GetMinimumCurveValue(TEXT("DG_FootPlant_L")) >= 0.85f)
                : AreFixturesAtMontageTime(PlantSeconds))
            {
                PreparePlantCapture();
            }
            else if (SecondsInStage() > PoseTimeoutSeconds)
        ...
        case EStage::WaitingForRelease:
            if ((bProductionMotionVisualCapture
                    ? AreFixturesAtPhase(EDGThrowPhase::Release)
                    : AreFixturesAtMontageTime(ReleaseSeconds))
        ...
        case EStage::WaitingForFollowThrough:
            if (bProductionMotionVisualCapture
                ? (AreFixturesAtPhase(EDGThrowPhase::FollowThrough)
                    && GetMinimumCurveValue(TEXT("DG_FollowThroughAlpha")) >= 0.85f)
                : AreFixturesAtMontageTime(FollowThroughSeconds))
            {
        ...
        case EStage::WaitingForRecovery:
            if (AreFixturesAtPhase(EDGThrowPhase::Recovery))
            {
                PrepareRecoveryCapture();
            }
            else if (SecondsInStage() > PoseTimeoutSeconds)
            {
                Fail(TEXT("exact Recovery phase was not reached"));
            }
            break;
...
bool ADiscGolfSession4VisualCaptureRunner::PreflightAssets()
{
    ...
    {
        if (bProductionMotionVisualCapture
            && FCString::Stricmp(Required.Label, TEXT("montage")) == 0)
        {
            continue;
        }
...
bool ADiscGolfSession4VisualCaptureRunner::SpawnProductionMotionFixtures()
{
    DestroyFixtures();
    FixtureLaunchCount = 0;
    FixtureLaunchGripTransforms.Reset();

    UDiscGolfCharacterProfile* Baseline =
        LoadObject<UDiscGolfCharacterProfile>(nullptr, Session4ProfilePaths[1]);
    if (!Baseline || !SpawnFixture(
            TEXT("Baseline"),
            Session4ProfilePaths[1],
            TEXT("AUTHORED_PROFILE"),
            Baseline->Body,
            Baseline->ThrowStyle,
            EDGHandedness::Right,
...
bool ADiscGolfSession4VisualCaptureRunner::RunHandednessBoundaryAssertion()
{
    ...
    FThrowCommand BoundFixtureCommand = FixtureThrowCommand;
    if (!BindSession4SelectedThrowProvenance(Fixture, BoundFixtureCommand))
    {
        return false;
    }
    ...
    const bool bUnexpectedStart = Fixture->TryStartAnimatedRHBHThrow(BoundFixtureCommand);
...
bool ADiscGolfSession4VisualCaptureRunner::BeginFixtureThrows()
{
    ...
    for (int32 Index = 0; Index < FixturePawns.Num(); ++Index)
    {
        ...
        if (!FixturePawns[Index] || !FixtureAdapters[Index]
            ...
            || !BindSession4SelectedThrowProvenance(
                FixturePawns[Index], BoundFixtureCommand))
        {
            return false;
        }
        FixtureThrowCommands.Add(BoundFixtureCommand);
        if (!FixturePawns[Index]->TryStartAnimatedRHBHThrow(BoundFixtureCommand))
        {
            return false;
        }
        ...
        {
            UAnimMontage* ActiveMontage = bProductionMotionVisualCapture
                ? Anim->GetCurrentActiveMontage()
                : ThrowMontage.Get();
            if (!ActiveMontage)
            {
                return false;
            ...
            if (bProductionMotionVisualCapture)
            {
                const FString ActivePath = ActiveMontage->GetPathName();
                const FString PawnSelectedPath =
                    FixturePawns[Index]->GetActiveRHBHThrowMontagePath();
                ...
                if (!ActivePath.Contains(
                    ...
                    || !ActivePath.EndsWith(
                        RevisionToken,
                        ESearchCase::CaseSensitive)
                    || PawnSelectedPath != ActivePath
                    || (!ActiveProductionMontagePath.IsEmpty()
                        && ActiveProductionMontagePath != ActivePath))
                {
                    UE_LOG(LogDiscGolfTour, Error,
                        TEXT("DG_SESSION4_VISUAL_CAPTURE: production montage mismatch expected Drive/*_%s anim=%s pawn=%s first=%s"),
                        *ExpectedProductionMotionRevision,
                        *ActivePath,
                        *PawnSelectedPath,
            ...
            Anim->Montage_SetPlayRate(ActiveMontage, 1.0f);
            if (bProductionMotionVisualCapture)
            {
    ...
    return !bProductionMotionVisualCapture
        || (AreFixturesAtPhase(EDGThrowPhase::Aim)
            && !ActiveProductionMontagePath.IsEmpty());
}
    ...
    const FTransform& GripWorldTransform)
{
    ...
    const bool bExactFixtureCommand = FixtureThrowCommands.ContainsByPredicate(
        [&AuthoritativeCommand](const FThrowCommand& Expected)
        {
            return Session4CommandsMatchExactly(AuthoritativeCommand, Expected);
        });
    return bExactFixtureCommand
...
bool ADiscGolfSession4VisualCaptureRunner::AreFixturesAtPhase(
    EDGThrowPhase TargetPhase) const
{
    if (FixturePawns.IsEmpty())
    {
        return false;
    }
    for (const ADiscGolferPawn* Fixture : FixturePawns)
    {
        const UDiscGolfThrowComponent* Throw = Fixture
            ? Fixture->FindComponentByClass<UDiscGolfThrowComponent>()
            : nullptr;
        if (!Throw || Throw->CurrentPhase != TargetPhase)
        {
            return false;
        }
    }
...
bool ADiscGolfSession4VisualCaptureRunner::ValidateExactPhaseGate(
    EDGThrowPhase TargetPhase) const
{
    return bProductionMotionVisualCapture && AreFixturesAtPhase(TargetPhase);
}
...
void ADiscGolfSession4VisualCaptureRunner::PrepareAimCapture()
{
    PauseFixtureMontages(true);
    const bool bExactPhase = ValidateExactPhaseGate(EDGThrowPhase::Aim);
    if (!bExactPhase)
    {
        Fail(TEXT("production motion Aim phase gate failed"));
        return;
    }
    MinimumPhaseCurveValue = -1.0f;
    PositionCompositeCamera(false);
    ShowEvidenceLabel(
        TEXT("SESSION 19 | PRODUCTION DRIVE | AIM"),
        FString::Printf(
            TEXT("BASELINE + SLIDER MAX  |  ACTIVE %s  |  NO GAMEPLAY MUTATION"),
            *ActiveProductionMontagePath));
    RequestCapture(
        0,
        TEXT("Baseline and SliderMax at the exact Aim lifecycle phase on the active production Drive montage."),
        TEXT("Aim"),
        TEXT("Three-quarter front composite"),
        EStage::WaitingForReachback);
    if (CaptureEvidence.IsValidIndex(0))
    {
        CaptureEvidence[0].bExactPhaseGateRequired = true;
        CaptureEvidence[0].bExactPhaseGatePassed = bExactPhase;
    }
    ValidateCompositeFraming(0);
}
...
{
    PauseFixtureMontages(true);
    const bool bExactPhase = !bProductionMotionVisualCapture
        || ValidateExactPhaseGate(EDGThrowPhase::ReachBack);
    if (!bExactPhase
        || !ValidatePhaseGate(TEXT("DG_ReachbackAlpha"), 0.90f))
    {
        Fail(TEXT("maximum-reachback exact phase or curve gate failed"));
        return;
    }
    PositionCompositeCamera(false);
    const FString ReachbackEvidenceSubtitle = bProductionMotionVisualCapture
        ? FString::Printf(
            TEXT("BASELINE + SLIDER MAX  |  DG_ReachbackAlpha MIN %.2f  |  NO GAMEPLAY DISCS"),
    ...
    ShowEvidenceLabel(
        bProductionMotionVisualCapture
            ? TEXT("SESSION 19 | PRODUCTION DRIVE | MAXIMUM REACHBACK")
            : TEXT("SESSION 4 | MAXIMUM REACHBACK"),
        ReachbackEvidenceSubtitle);
    const int32 CaptureIndex = bProductionMotionVisualCapture ? 1 : 2;
    RequestCapture(
        CaptureIndex,
        bProductionMotionVisualCapture
            ? TEXT("Baseline and SliderMax at exact ReachBack with DG_ReachbackAlpha >= 0.90 on the active production Drive montage.")
            : TEXT("All three profiles at the authored maximum-reachback phase."),
        TEXT("ReachBack"),
        TEXT("Three-quarter front composite"),
        EStage::WaitingForPlant);
    if (bProductionMotionVisualCapture
        && CaptureEvidence.IsValidIndex(CaptureIndex))
    {
        CaptureEvidence[CaptureIndex].Evidence =
            TEXT("Baseline and SliderMax at exact ReachBack with DG_ReachbackAlpha >= 0.90 on the active production Drive montage.");
        CaptureEvidence[CaptureIndex].bExactPhaseGateRequired = true;
        CaptureEvidence[CaptureIndex].bExactPhaseGatePassed = bExactPhase;
    }
    ValidateCompositeFraming(CaptureIndex);
}
...
void ADiscGolfSession4VisualCaptureRunner::PreparePlantCapture()
{
    PauseFixtureMontages(true);
    const bool bExactPhase = !bProductionMotionVisualCapture
        || ValidateExactPhaseGate(EDGThrowPhase::Plant);
    const bool bContactPassed = ValidatePlantContactGate();
    if (!bExactPhase || !bContactPassed)
    {
    ...
    ShowEvidenceLabel(
        bProductionMotionVisualCapture
            ? TEXT("SESSION 19 | PRODUCTION DRIVE | PLANT / BRACE")
            : TEXT("SESSION 4 | PLANT / BRACE"),
        bProductionMotionVisualCapture
            ? TEXT("BASELINE + SLIDER MAX  |  LEFT PLANT CONTACT GATED  |  DG_BraceAlpha >= 0.85")
            : TEXT("LEFT PLANT CONTACT GATED  |  DG_BraceAlpha >= 0.85  |  ALL THREE PROFILES"));
    const int32 CaptureIndex = bProductionMotionVisualCapture ? 2 : 3;
    RequestCapture(
        CaptureIndex,
        bProductionMotionVisualCapture
            ? TEXT("Baseline and SliderMax at exact Plant with brace, left-foot contact, and no-gameplay-mutation gates.")
    ...
    {
        CaptureEvidence[CaptureIndex].bContactGateRequired = true;
        CaptureEvidence[CaptureIndex].bContactGatePassed = bContactPassed;
        CaptureEvidence[CaptureIndex].bExactPhaseGateRequired =
            bProductionMotionVisualCapture;
        CaptureEvidence[CaptureIndex].bExactPhaseGatePassed = bExactPhase;
        if (bProductionMotionVisualCapture)
        {
...
void ADiscGolfSession4VisualCaptureRunner::PrepareReleaseCapture()
{
    PauseFixtureMontages(true);
    const bool bExactPhase = !bProductionMotionVisualCapture
        || ValidateExactPhaseGate(EDGThrowPhase::Release);
    const bool bReleasePassed = ValidateReleaseGate();
    if (!bExactPhase || !bReleasePassed)
    {
    ...
    ShowEvidenceLabel(
        bProductionMotionVisualCapture
            ? TEXT("SESSION 19 | PRODUCTION DRIVE | EXACT RELEASE PHASE")
            : TEXT("SESSION 4 | EXACT RELEASE PHASE"),
        bProductionMotionVisualCapture
            ? TEXT("BASELINE + SLIDER MAX  |  2 VALID GRIP CALLBACKS  |  0 GAMEPLAY DISCS / 0 STROKES")
            : TEXT("3 FIXTURES  |  3 VALID GRIP CALLBACKS  |  0 GAMEPLAY DISCS / 0 STROKES"));
    const int32 CaptureIndex = bProductionMotionVisualCapture ? 3 : 4;
    RequestCapture(
        CaptureIndex,
        bProductionMotionVisualCapture
            ? TEXT("Baseline and SliderMax at exact Release after one validation-only grip callback each.")
    ...
    if (CaptureEvidence.IsValidIndex(CaptureIndex))
    {
        CaptureEvidence[CaptureIndex].bReleaseGateRequired = true;
        CaptureEvidence[CaptureIndex].bReleaseGatePassed = bReleasePassed;
        CaptureEvidence[CaptureIndex].bExactPhaseGateRequired =
            bProductionMotionVisualCapture;
        CaptureEvidence[CaptureIndex].bExactPhaseGatePassed = bExactPhase;
        if (bProductionMotionVisualCapture)
...
void ADiscGolfSession4VisualCaptureRunner::PrepareFollowThroughCapture()
{
    PauseFixtureMontages(true);
    const bool bExactPhase = !bProductionMotionVisualCapture
        || ValidateExactPhaseGate(EDGThrowPhase::FollowThrough);
    if (!bExactPhase
        || !ValidatePhaseGate(TEXT("DG_FollowThroughAlpha"), 0.85f))
    {
    ...
    ShowEvidenceLabel(
        bProductionMotionVisualCapture
            ? TEXT("SESSION 19 | PRODUCTION DRIVE | FOLLOW-THROUGH")
            : TEXT("SESSION 4 | FOLLOW-THROUGH"),
        bProductionMotionVisualCapture
            ? TEXT("BASELINE + SLIDER MAX  |  SAME ACTIVE PRODUCTION MONTAGE  |  VALIDATION-ONLY RELEASES")
            : TEXT("ALL THREE PROFILES  |  SAME RHBH MONTAGE  |  VALIDATION-ONLY RELEASES"));
    const int32 CaptureIndex = bProductionMotionVisualCapture ? 4 : 5;
    RequestCapture(
        CaptureIndex,
        bProductionMotionVisualCapture
            ? TEXT("Baseline and SliderMax at exact FollowThrough with DG_FollowThroughAlpha >= 0.85.")
...
void ADiscGolfSession4VisualCaptureRunner::PrepareRecoveryCapture()
{
    PauseFixtureMontages(true);
    const bool bExactPhase = ValidateExactPhaseGate(EDGThrowPhase::Recovery);
    if (!bExactPhase || FixtureLaunchCount != FixturePawns.Num())
    {
        Fail(TEXT("production motion Recovery phase or exactly-once release gate failed"));
        return;
    }
    MinimumPhaseCurveValue = -1.0f;
    PositionCompositeCamera(false);
    ShowEvidenceLabel(
        TEXT("SESSION 19 | PRODUCTION DRIVE | RECOVERY"),
        TEXT("BASELINE + SLIDER MAX  |  EXACT RECOVERY  |  RELEASE COUNT UNCHANGED"));
    RequestCapture(
        5,
        TEXT("Baseline and SliderMax at exact Recovery after one validation-only release each."),
        TEXT("Recovery"),
        TEXT("Three-quarter front composite"),
        EStage::Finished);
    if (CaptureEvidence.IsValidIndex(5))
    {
        CaptureEvidence[5].bExactPhaseGateRequired = true;
        CaptureEvidence[5].bExactPhaseGatePassed = bExactPhase;
        CaptureEvidence[5].bReleaseGateRequired = true;
        CaptureEvidence[5].bReleaseGatePassed =
            FixtureLaunchCount == FixturePawns.Num();
    }
    ...
    bool bExtremeView)
{
    ...
    // The legacy Session 4 frame fits three fixtures. Production evidence has
    // two fixtures and needs enough on-screen pixels to judge the dynamic pose.
    const float Distance = bExtremeView
        ? 1050.0f
        : (bProductionMotionVisualCapture ? 950.0f : 1325.0f);
    ...
    int32 CaptureIndex)
{
    ...
    for (const USkeletalMeshComponent* Mesh : FixtureMeshes)
    {
        ...
        Record.bCameraOutsideSubjects &=
            !Mesh->Bounds.GetBox().ExpandBy(35.0f).IsInside(
                CaptureCamera->GetActorLocation());
        const bool bPoseFiniteAndPlausible = bProductionMotionVisualCapture
            ? IsDynamicPoseFiniteAndPlausible(Mesh)
            : IsPoseFiniteAndPlausible(Mesh);
        Record.bFiniteTransforms &= bPoseFiniteAndPlausible;
        UE_LOG(LogDiscGolfTour, Display,
            TEXT("DG_SESSION4_POSE_GATE: capture=%d production=%s pass=%s bounds_radius_cm=%.3f"),
            CaptureIndex + 1,
            bProductionMotionVisualCapture ? TEXT("true") : TEXT("false"),
            bPoseFiniteAndPlausible ? TEXT("true") : TEXT("false"),
    ...
    if (Record.Phase == TEXT("Aim")
        || Record.Phase == TEXT("ReachBack")
        || Record.Phase == TEXT("Plant"))
    {
    ...
    // A deep reachback/follow-through can be much wider than it is tall in
    // screen space. The 1920x1080 production frame remains readable at 14% of
    // frame height while the independent full-body and landmark gates prevent
    // cropping; the legacy upright three-fixture contract stays at 22%.
    const float MinimumReadableScreenHeight = bProductionMotionVisualCapture
        ? 0.14f
        : 0.22f;
    Record.bReadable = Record.bAllSubjectsFramed
        && Record.MinSubjectScreenHeightFraction >= MinimumReadableScreenHeight
...
bool ADiscGolfSession4VisualCaptureRunner::IsDynamicPoseFiniteAndPlausible(
    const USkeletalMeshComponent* Mesh) const
{
    if (!Mesh || !Mesh->GetSkeletalMeshAsset()
        || !Mesh->GetComponentTransform().IsValid()
    ...
    {
        const FVector Location = GetBoneLocation(Mesh, Bone);
        if (!IsSession4FiniteVector(Location)
            || FVector::Dist(Location, Pelvis) > 400.0f)
        {
            return false;
        }
    }

    // Height-from-ground and vertical bounds are pose-dependent: a braced or
    // rotated athlete is not a neutral upright anthropometry sample. Keep the
    // finite and invariant bone-chain gates that still apply during motion.
    const FBodyMetrics Metrics = MeasureBody(Mesh);
    return FMath::IsFinite(Metrics.WingspanChainCm)
...
bool ADiscGolfSession4VisualCaptureRunner::ValidateProfileOrdering() const
{
    if (bProductionMotionVisualCapture)
    {
        if (ProfileEvidence.Num() != 2)
        {
            return false;
        }
        const FBodyMetrics& Baseline = ProfileEvidence[0].NeutralMetrics;
        const FBodyMetrics& Maximum = ProfileEvidence[1].NeutralMetrics;
        return Baseline.bFiniteAndPlausible
            && Maximum.bFiniteAndPlausible
            && Baseline.HeightLandmarkCm < Maximum.HeightLandmarkCm
            && Baseline.WingspanChainCm < Maximum.WingspanChainCm
            && Baseline.ShoulderWidthCm < Maximum.ShoulderWidthCm
            && Baseline.HandLengthCm < Maximum.HandLengthCm;
    }
...
int32 ADiscGolfSession4VisualCaptureRunner::GetExpectedCaptureCount() const
{
    return bProductionMotionVisualCapture
        ? UE_ARRAY_COUNT(ProductionMotionCaptureFilenames)
        : UE_ARRAY_COUNT(Session4CaptureFilenames);
}

const TCHAR* ADiscGolfSession4VisualCaptureRunner::GetCaptureFilename(
    int32 CaptureIndex) const
{
    ...
    return bProductionMotionVisualCapture
        ? ProductionMotionCaptureFilenames[CaptureIndex]
        : Session4CaptureFilenames[CaptureIndex];
}
    ...
    bool bUseCaptureCamera)
{
    ...
    UE_LOG(LogDiscGolfTour, Display,
        TEXT("DG_SESSION4_VISUAL_CAPTURE: FRAMED %d/%d %s"),
        CaptureIndex + 1,
...
bool ADiscGolfSession4VisualCaptureRunner::PollPendingCapture()
{
    if (!bScreenshotIssued)
    {
        ...
        FScreenshotRequest::RequestScreenshot(
            ...
            true);
        UE_LOG(LogDiscGolfTour, Display,
            TEXT("DG_SESSION4_VISUAL_CAPTURE: REQUEST %d/%d %s"),
    ...
    if (FileSize >= MinimumReadablePngBytes)
    {
        ...
        UE_LOG(LogDiscGolfTour, Display,
            TEXT("DG_SESSION4_VISUAL_CAPTURE: CAPTURED %d/%d 1920x1080 bytes=%lld sha1=%s"),
            CompletedIndex + 1,
            GetExpectedCaptureCount(),
            Record.Bytes,
            *Record.Sha1);
    ...
    if (bProductionMotionVisualCapture)
    {
        switch (CaptureIndex)
        {
            ...
            case 5:
                ...
                break;
            default:
                Fail(TEXT("unknown production-motion capture completion index"));
                break;
        }
        return;
    }
    ...
    const FString& Error)
{
    if (bProductionMotionVisualCapture)
    {
        WriteProductionMotionManifest(bPassed, Error);
        return;
    }
    ...
    for (const FCaptureEvidence& Capture : CaptureEvidence)
    {
        ...
        Object->SetBoolField(
            TEXT("exact_phase_gate_required"),
            Capture.bExactPhaseGateRequired);
        Object->SetBoolField(
            TEXT("exact_phase_gate_passed"),
            Capture.bExactPhaseGatePassed);
...
void ADiscGolfSession4VisualCaptureRunner::WriteProductionMotionManifest(
    bool bPassed,
    const FString& Error)
{
    TSharedRef<FJsonObject> Root = MakeShared<FJsonObject>();
    Root->SetStringField(
        TEXT("schema"),
        TEXT("DiscGolfTour.Session19ProductionMotionVisualEvidence.v1"));
    Root->SetStringField(TEXT("status"), bPassed ? TEXT("PASS") : TEXT("FAIL"));
    Root->SetStringField(
        TEXT("scope"),
        TEXT("ACTIVE_PRODUCTION_DRIVE_PHASE_VISUAL_EVIDENCE_ONLY"));
    Root->SetStringField(TEXT("engine_version"), FEngineVersion::Current().ToString());
    Root->SetStringField(TEXT("output_directory"), OutputDirectory);
    Root->SetStringField(TEXT("error"), Error);
    ...
    Root->SetStringField(
        ...
        PossessedGolfer
            ? PossessedGolfer->GetProductionMotionCandidateRevision()
            : FString());
    ...
    Root->SetStringField(
        ...
        RequestedMotionCandidate);
    Root->SetStringField(
        TEXT("active_production_montage"),
        ActiveProductionMontagePath);
    Root->SetBoolField(
        TEXT("active_montage_is_expected_drive_revision"),
        !ActiveProductionMontagePath.IsEmpty()
            && ActiveProductionMontagePath.Contains(
                TEXT("/Game/DiscGolf/Animation/ProductionMotion/Drive/"),
                ESearchCase::CaseSensitive)
            && ActiveProductionMontagePath.EndsWith(
                FString::Printf(TEXT("_%s"), *ExpectedProductionMotionRevision),
                ESearchCase::CaseSensitive));
    Root->SetStringField(TEXT("command_line"), FCommandLine::Get());

    TArray<TSharedPtr<FJsonValue>> RequiredPhases;
    for (const TCHAR* Phase : ProductionMotionRequiredPhases)
    {
        RequiredPhases.Add(MakeShared<FJsonValueString>(Phase));
    }
    Root->SetArrayField(TEXT("required_phase_order"), RequiredPhases);
    ...
    for (const FProfileEvidence& Profile : ProfileEvidence)
    {
        ...
        Object->SetObjectField(TEXT("body_profile"), BodyToJson(Profile.Body));
        Object->SetObjectField(TEXT("throw_style"), StyleToJson(Profile.ThrowStyle));
        Object->SetStringField(TEXT("handedness"), TEXT("Right"));
        Object->SetNumberField(
            TEXT("height_head_to_ground_landmark_cm"),
            Profile.NeutralMetrics.HeightLandmarkCm);
        Object->SetNumberField(
        ...
        Object->SetNumberField(
            ...
            Profile.NeutralMetrics.HandLengthCm);
        Object->SetBoolField(
            TEXT("finite_neutral_metrics"),
            Profile.NeutralMetrics.bFiniteAndPlausible
                && Profile.bFiniteNeutralPose);
        Object->SetNumberField(
            TEXT("release_commit_count"),
            Profile.ReleaseCommitCount);
        Object->SetBoolField(TEXT("same_skeletal_mesh"), Profile.bSameSkeletalMesh);
        Object->SetBoolField(TEXT("same_skeleton"), Profile.bSameSkeleton);
        Object->SetBoolField(TEXT("same_animation_blueprint"), Profile.bSameAnimBlueprint);
        Profiles.Add(MakeShared<FJsonValueObject>(Object));
    ...
    Isolation->SetNumberField(TEXT("baseline_world_disc_count"), BaselineWorldDiscCount);
    Isolation->SetNumberField(TEXT("final_world_disc_count"), CountWorldDiscs());
    ...
    Isolation->SetNumberField(
        TEXT("validation_only_fixture_launch_callbacks"),
        FixtureLaunchCount);
    Isolation->SetBoolField(TEXT("gameplay_disc_or_stroke_created"), false);
    ...
    Writes->SetStringField(
        TEXT("allowed_outputs"),
        TEXT("Exactly six PNG files and this JSON manifest under Saved/CharacterFramework/Screenshots/Session19_ProductionMotion."));
    Writes->SetBoolField(TEXT("package_save_calls"), false);
    Writes->SetBoolField(TEXT("level_save_calls"), false);
    Root->SetObjectField(TEXT("persistent_write_guard"), Writes);

    Root->SetBoolField(
        TEXT("manual_visual_review_required"),
        true);
    Root->SetStringField(
        TEXT("manual_visual_review_scope"),
        TEXT("Biomechanical silhouette, foot sliding, limb intersections, disc grip, balance, and phase-to-phase continuity must be reviewed from the six rendered PNGs; this automated pass proves identity, lifecycle, framing, and isolation only."));
...
void ADiscGolfSession4VisualCaptureRunner::Pass()
{
    ...
    bool bCaptureFilesPassed =
        CaptureEvidence.Num() == GetExpectedCaptureCount();
    for (const FCaptureEvidence& Capture : CaptureEvidence)
    {
        bCaptureFilesPassed &= Capture.Width == ExpectedWidth
            ...
            && (!Capture.bExactPhaseGateRequired || Capture.bExactPhaseGatePassed);
    }
    const bool bRegularReleaseCountsPassed = bProductionMotionVisualCapture
        ? (ProfileEvidence.Num() == 2
            && ProfileEvidence[0].ReleaseCommitCount == 1
            && ProfileEvidence[1].ReleaseCommitCount == 1
            && FixtureLaunchCount == 2
            && FixtureLaunchGripTransforms.Num() == 2)
        : (ProfileEvidence.Num() >= 3
            && ProfileEvidence[0].ReleaseCommitCount == 1
            && ProfileEvidence[1].ReleaseCommitCount == 1
    ...
    if (!bCaptureFilesPassed
        ...
        || !bNoGameplayMutation
        || !bNoWrites
        || (!bProductionMotionVisualCapture && !bCreatorLiveAdjustmentApplied)
        || (!bProductionMotionVisualCapture && !bCreatorCancelRestored)
        || (!bProductionMotionVisualCapture && !bHandednessBoundaryPassed)
        || (bProductionMotionVisualCapture
            && (ActiveProductionMontagePath.IsEmpty()
                || !PossessedGolfer
                || PossessedGolfer->GetProductionMotionCandidateRevision()
    ...
    UE_LOG(LogDiscGolfTour, Display,
        TEXT("DG_SESSION4_VISUAL_CAPTURE: PASS mode=%s captures=%d profiles=%d fixture_releases=%d active_montage=%s manifest=%s"),
        bProductionMotionVisualCapture ? TEXT("production_motion") : TEXT("session4"),
        CaptureEvidence.Num(),
        ProfileEvidence.Num(),
        FixtureLaunchCount,
