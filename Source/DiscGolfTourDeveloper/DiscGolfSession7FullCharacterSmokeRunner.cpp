...
void ADiscGolfSession7FullCharacterSmokeRunner::Start()
{
    ...
    {
        Fail(TEXT("Session 7 runner did not start from the current schema-10 profile"));
        return;
    }
    ...
    FString& OutError)
{
    ...
    {
        OutError = TEXT("schema-8 fixture did not migrate exactly once to schema 10");
        return false;
    }
    ...
    {
        OutError = TEXT("migrated schema-10 full character did not survive disk reload");
        return false;
    }
    return true;
}
    ...
    FString& OutError)
{
    ...
    if (!Reloaded || Reloaded->SaveSchemaVersion != DiscGolfSaveSchema::CurrentVersion
        || !DiscGolfFullCharacterRuntime::AreCustomizationsEquivalent(
            ...
            Golfer->GetCurrentFullCharacterCustomization()))
    {
        OutError = TEXT("Randomize -> Apply did not reload the complete schema-10 payload from disk");
        return false;
    }
